...
export const useScore = (totalNotes: MaybeRefOrGetter<number>) => {
  const { judgementScores, MAX_SCORE, MOD_DIVIDER, MOD_MULTIPLIER, MAX_BONUS, MIN_BONUS } = useScoreStore()
  const score = ref(0)
  let bonus = MAX_BONUS

  const calculateBaseScore = (j: Judgement) => {
    return (MAX_SCORE * MOD_MULTIPLIER * 0.5 / toValue(totalNotes)) * (judgementScores[j].hitValue / 320)
  }
  ...
  const calculateBonusScore = (j: Judgement) => {
    ...
    return (MAX_SCORE * MOD_MULTIPLIER * 0.5 / toValue(totalNotes)) * (hitBonusValue * Math.sqrt(bonus) / 320)
  }
