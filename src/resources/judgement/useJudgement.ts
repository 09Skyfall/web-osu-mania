...
const calculateWindows = (scrollSpeed: number, colHeight: number) => {
  return mapValues(JUDGEMENT_WINDOWS, (jw) => ({
    top: colHeight - (scrollSpeed * jw) / 2,
    bottom: colHeight + (scrollSpeed * jw) / 2
  }))
}
...
export const useJudgement = (
  ...
  const judged: string[] = []
  const windows = calculateWindows(SCROLL_SPEED, COL_HEIGHT)
  ...
  const drawJudgementLines: CanvasAnimationFunction = (ctx) => {
    Object.values(windows).forEach(({ top }) => {
      ctx.fillStyle = "red"
  ...
  const onKeyPressChange = (pressed: boolean) => {
    const toBeJudged = notes.value
      .filter(note => inRange(note.y, windows.MISS.top, windows.MISS.bottom + 1))
  ...
  const judge = (note?: CanvasNote) => {
    if (!note) { // counts as a late miss
      judgementService.add("MISS")
    } else {
      if (note.type === NOTE_TYPE.TAIL) judgingLongNote = false
      const jw = (Object.keys(windows) as Judgement[]).find(
        (jw) => inRange(note.y, windows[jw].top, windows[jw].bottom)
      )
