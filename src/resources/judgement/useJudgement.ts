...
export const useJudgement = (
  ...
  const judged: string[] = []
  const windows = computed(() => mapValues(JUDGEMENT_WINDOWS, (jw) => ({
  ...
  const judge = (note?: CanvasNote) => {
    if (!note) { // counts as a late miss
      ...
      const jw = (Object.keys(windows.value) as Judgement[]).find(
        (jw) => inRange(note.y, windows.value[jw].top, windows.value[jw].bottom)
      )
