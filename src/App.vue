...
import { BlobReader, BlobWriter, getMimeType, TextWriter, ZipReader } from '@zip.js/zip.js';
import { ref } from 'vue';
...
const map = ref<{ song?: string; cols: ColumnProps[] }>({ cols: [
  { hitKey: "a", notes: [] },
  { hitKey: "s", notes: [] },
  { hitKey: "k", notes: [] },
  { hitKey: "l", notes: []},
...
const onSelectFile = async (e: Event) => {
  ...
  // AUDIO
  const audioFilename = new RegExp(/^AudioFilename: (?<audioFile>.*)$/, 'gm').exec(text)?.groups?.audioFile
  ...
  const audioBlob = await audioFile?.getData(new BlobWriter(getMimeType(audioFilename?.split(".")[1])))
